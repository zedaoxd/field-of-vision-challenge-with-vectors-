...
public class CharacterMovement : MonoBehaviour
{
    ...
    public void SetInput(float horizontal, float vertical)
    {
        ...
        if (horizontal != 0 || vertical != 0) targetRotation = Quaternion.LookRotation(input);
    }
    ...
    private void CheckCollisionHorizontal()
    {
        ...
        for (int i = 0; i < horizontalIterations; i++)
        {
            ...
            {
                var hit = hits[j];
                var projetedVelocity = Vector3.ProjectOnPlane(Velocity, hit.normal);
                if (projetedVelocity != Vector3.zero)
                {
                    Velocity = projetedVelocity.normalized * Velocity.magnitude;
                    break;
                }
            }
        }
